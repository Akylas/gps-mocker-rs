 ...
 */
export class MapDelatinHeightNode extends MapHeightNode
{
	/**
	...
	public static geometry = new MapNodeGeometry(1, 1, MapDelatinHeightNode.geometrySize, MapDelatinHeightNode.geometrySize);
	...
	public constructor(parentNode: MapDelatinHeightNode = null, mapView: MapView = null, location: number = MapNode.root, level: number = 0, x: number = 0, y: number = 0, {elevationDecoder = [256* 255, 255, 1 / 256* 255, -32768], meshMaxError = 50, exageration = 1}: {elevationDecoder?: number[], meshMaxError?: number | Function, exageration?: number} = {})
	{
		super(parentNode, mapView, location, level, x, y, MapDelatinHeightNode.geometry, MapDelatinHeightNode.prepareMaterial(new MeshPhongMaterial({
			map: MapDelatinHeightNode.emptyTexture,
			color: 0xFFFFFF,
			side: DoubleSide
		}), level, exageration));
	...
	{
		material.userData = {
			heightMap: {value: MapDelatinHeightNode.emptyTexture},
			drawNormals: {value: 0},
			drawBlack: {value: 0},
	 ...
	 */
	public static getMeshAttributes(tin: Delatin, tileSize: number, bounds: number[]): {position: {value: Float32Array, size: number}, uv: {value: Float32Array, size: number}} // NORMAL: {}, - optional, but creates the high poly look with lighting}
	{
		const {coords, triangles} = tin; // get vertices and triangles of the mesh
		const gridSize = tileSize + 1;
		const numOfVerticies = coords.length / 2;
		...
		for (let i = 0; i < numOfVerticies; i++) 
		{
			...
			positions[3 * i + 1] = -tin.heightAt(x, y);
	...
	public async onHeightImage(image: HTMLImageElement): Promise<void> 
	{
		if (image) 
		{
			...
			const values = new Float64Array(tileSize * tileSize);
			const elevationDecoder = this.elevationDecoder;
			for (let i = 0; i < values.length; i++) 
			{
				const r = data[4 * i];
				const g = data[4 * i + 1];
				const b = data[4 * i + 2];
				values[i] = Math.round((r * elevationDecoder[0]/255 + g * elevationDecoder[1] /255+ b * elevationDecoder[2]/255 + elevationDecoder[3]) * 10) / 10;
			}
			const tin = new Delatin(values, tileSize, tileSize);
			tin.run(100 * Math.pow(4, 10 - this.level)); 
			// tin.run(0.3); // run mesh refinement until max error is less than 0.3
			// const {coords, triangles} = tin; // get vertices and triangles of the mesh
	
			const attributes = MapDelatinHeightNode.getMeshAttributes(tin, tileSize, [-0.5, -0.5, 0.5, 0.5]);
			this.geometry = new BufferGeometry();
			this.geometry.setIndex(new Uint32BufferAttribute(tin.triangles, 1));
